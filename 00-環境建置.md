...
- 登入 OpenAI（ChatGPT 帳號或 OpenAI API Key）
- 驗證一切正常
...
- [ ] OpenAI 帳號（可用 ChatGPT 帳號或 OpenAI API Key 登入）
- [ ] 電腦有網路連線
...
3. 安裝完成、開啟 app
4. **Sign in with ChatGPT** 或 **Sign in with API Key**
   - ChatGPT 帳號是建議路徑
   - API Key 也可用，但部分功能可能受限
5. 登入後選擇一個專案資料夾
6. 看到 Codex 主畫面並可開始對話 = OK

> 💡 **不在 Mac/Win**？改用 IDE 擴充或 CLI。IDE 擴充請依官方 Codex IDE 文件安裝；CLI 請依官方 Codex CLI 文件安裝。三者設定共用，後面所有懶人包都通用。

---

### 步驟二：安裝 Node.js（如果未安裝或版本 < 18）

> 💡 不是 Codex Desktop app 本體一定要的，但**很多 MCP / 本機整合會用到 Node.js**。如果你後面要接 Obsidian、Supabase、Firebase 等工具，保留這步比較穩。
...
| 問題 | 解法 |
|------|------|
| Desktop app 開不起來 | macOS：右鍵→開啟（首次繞 Gatekeeper）；Windows：以系統管理員身分執行一次 |
| Sign in with ChatGPT 卡住 | 改用 Sign in with API Key；或檢查瀏覽器是否封鎖登入跳轉 |
| Linux 沒有 Desktop app | 改用官方 Codex IDE Extension 或 Codex CLI |
...
- [Codex 官方文件總覽](https://developers.openai.com/codex)
